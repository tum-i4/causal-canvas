...
class App extends Component<any, ICausalCanvasState> {
	...
	handelGraphImport = async (data: IGraphImportData) => {
		...
		switch (data.type) {
			...
			case GraphImportType.CausalModel: graph = await handelCausalModelImport(JSON.parse(data.src), general, width, height); break;
		}
...
const handelCausalModelImport = (graph: IGraph, general, width, height) => {
	if (graph.id !== '-1') {
		return graph;
	}

	return layoutGraph(graph, general, width, height);
}